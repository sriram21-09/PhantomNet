...
class StressTestSimulator:
    ...
    def _generate_attack_payload(self) -> ThreatInput:
        ...
        attack_ips = [
            f"203.0.113.{random.randint(1, 200)}" for _ in range(self.num_attackers)
        ]
    ...
    def run(self):
        ...
        logger.info(
            f"Starting Stress Test: {self.num_attackers} Workers pumping {self.target_events} vectors."
        )
