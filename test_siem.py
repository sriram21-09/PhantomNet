...
def test_export():
    ...
    print(
        f"Time taken to send 1000 Splunk events: {splunk_elapsed:.2f}s (Perf: {1000/splunk_elapsed:.2f} events/sec)"
    )
    assert (
        MockSIEMHandler.splunk_events == 1000
    ), f"Expected 1000 Splunk events, got {MockSIEMHandler.splunk_events}"
    ...
    print(
        f"Time taken to send 1000 ELK events: {elk_elapsed:.2f}s (Perf: {1000/elk_elapsed:.2f} events/sec)"
    )
    assert (
        MockSIEMHandler.elk_events == 1000
    ), f"Expected 1000 ELK events, got {MockSIEMHandler.elk_events}"
