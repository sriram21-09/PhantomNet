...
# Targets defined by SD-N topology - modified for local Docker Compose testing
TARGETS = {"ssh": "localhost", "smtp": "localhost", "backend": "localhost"}
PORTS = {
    "ssh": 2222,  # Using simulated ports mapped in the honeypots
    "smtp": 2525,
}
...
class AttackSimulator:
    ...
    def simulate_port_scan(self, target_ip, start_port=20, end_port=100):
        logging.info(
            f"[*] Starting SYN/Connect Scan on {target_ip} ({start_port}-{end_port})"
        )
    ...
    def simulate_ssh_brute_force(self):
        ...
        for user, password in credentials:
            try:
                ...
                ssh.connect(
                    target, port=port, username=user, password=password, timeout=2
                )
            ...
            time.sleep(
                0.5
            )  # Slight delay to avoid completely crashing local test environments
    ...
    def validate_detection(self):
        ...
        try:
            # Short wait to allow ML pipeline to process logs
            logging.info(
                "    -> Waiting 10 seconds for log ingestion & ML pipeline scoring..."
            )
            time.sleep(10)
            ...
            if response.status_code == 200:
                ...
                malicious_score = sum(
                    1
                    for log in logs
                    if log.get("ai_analysis", {}).get("prediction") == "MALICIOUS"
                    or log.get("ai_analysis", {}).get("threat_score", 0) > 0.5
                )

                logging.info(
                    f"[+] Successfully fetched {len(logs)} recent traffic logs from backend."
                )
                logging.info(
                    f"[+] Out of the {len(logs)} recent packets, {malicious_score} were flagged as MALICIOUS by the ML Model."
                )

                if malicious_score > 0:
                    logging.info(
                        "[!] VALIDATION PASSED: ML Model successfully identified the simulated attacks."
                    )
                else:
                    logging.warning(
                        "[-] VALIDATION FAILED: Backend did not classify recent events as MALICIOUS. Check Model thresholds or Logstash pipeline."
                    )
            else:
            ...
            logging.error(
                f"[-] Could not connect to Backend API at {BACKEND_API}. Ensure backend is up."
            )
        except Exception as e:
...
if __name__ == "__main__":
    print("""
    =============================================
     PhantomNet Attack Simulation Framework (Local)
     -> Target: Localhost Docker Compose Honeypots
    =============================================
    """)
    ...
    # Note: We won't validate immediately, we will do it through our E2E verification
    print("\n[+] Local Simulation Campaign Finished.")
