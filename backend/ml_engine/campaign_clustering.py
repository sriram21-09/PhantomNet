...
class CampaignClusterer:
    ...
    def identify_campaigns(self, hours_back: int = 24) -> Dict[str, Any]:
        ...
        """
        logger.info(f"Extracting attack groups from last {hours_back} hours.")
        ...
        try:
            ...
            for idx, cluster_id in enumerate(predictions):
                ...
                if log_ref.dst_port: c["target_ports"].add(log_ref.dst_port)
                if log_ref.protocol: c["protocols"].add(log_ref.protocol)
                c["event_count"] += 1
                
                if log_ref.timestamp < c["start_time"]: c["start_time"] = log_ref.timestamp
                if log_ref.timestamp > c["end_time"]: c["end_time"] = log_ref.timestamp
            ...
            logger.error(f"Error during campaign clustering: {e}")
