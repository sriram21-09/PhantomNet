...
class RetrainingPipeline:
    ...
    def fetch_training_data(self, days=30) -> pd.DataFrame:
        ...
        with self._get_db() as db:
            ...
            if len(logs) < 1000:
                logger.warning(
                    f"Insufficient data volume ({len(logs)} rows) for scheduled retraining."
                )
                return pd.DataFrame()
            ...
            logger.info(
                f"Loaded {len(df)} records. Class balance: \n{df['is_attack'].value_counts()}"
            )
            return df
        ...
        if score > 0.85:  # Threshold limit to prevent deployment of bad weights
            ...
            if os.path.exists(self.rf_model_path):
                backup_path = os.path.join(
                    self.models_dir, f"attack_predictor_backup_{timestamp}.pkl"
                )
            ...
            logger.warning(
                "Newly trained model failed accuracy threshold check. Discarding weights."
            )
            return False
