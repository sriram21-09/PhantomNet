...
class AdvancedPatternDetector:
    ...
    def detect_distributed_brute_force(self, time_window_mins: int = 15, target_port: int = 2222, min_sources: int = 3, min_attempts: int = 20) -> List[Dict[str, Any]]:
        """
        ...
        try:
            results = (
                self.db.query(
                    PacketLog.dst_ip,
                    PacketLog.dst_port,
                    func.count(func.distinct(PacketLog.src_ip)).label('distinct_sources'),
                    func.count(PacketLog.id).label('total_attempts')
                )
                .filter(
                    PacketLog.timestamp >= since,
                    PacketLog.dst_port == target_port
                )
    ...
    def detect_low_and_slow_scan(self, time_window_hours: int = 24, min_ports: int = 50, max_rate_per_min: float = 2.0) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            results = (
                self.db.query(
                    PacketLog.src_ip,
                    func.count(func.distinct(PacketLog.dst_port)).label('distinct_ports'),
    ...
    def run_all_checks(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "distributed_brute_force_ssh": self.detect_distributed_brute_force(target_port=2222),
