...
class SchedulerService:
    ...
    def add_report_job(self, report):
        trigger = self._get_trigger(report.frequency, report.schedule_time, report.day_of_week)
        ...
        logger.info(f"Added job for report {report.id} ({report.name}) at {report.schedule_time}")
    ...
    def _get_trigger(self, frequency, schedule_time, day_of_week=None):
        ...
        if frequency == "Daily":
            ...
            return CronTrigger(day_of_week=day_of_week or 'mon', hour=hour, minute=minute)
    ...
    def run_scheduled_report(self, report_id):
        ...
        try:
            ...
            filters = json.loads(report.filters.replace("'", '"')) if isinstance(report.filters, str) else report.filters
