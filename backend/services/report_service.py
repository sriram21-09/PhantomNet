...
class ReportService:
    ...
    def get_report_data(self, template_type: str, filters: dict):
        ...
        if isinstance(requested_sections, str):
            requested_sections = [
                s.strip() for s in requested_sections.split(",") if s.strip()
            ]
        ...
        for section in requested_sections:
            if section == "Executive Summary":
                data["sections"]["Executive Summary"] = self._get_executive_summary(
                    start_time, filters
                )
            elif section == "Attack Timeline":
                data["sections"]["Attack Timeline"] = self._get_attack_timeline(
                    start_time, filters
                )
            elif section == "Geographic Distribution":
                data["sections"]["Geographic Distribution"] = (
                    self._get_geographic_distribution(start_time, filters)
                )
            elif section == "Top Attackers":
                data["sections"]["Top Attackers"] = self._get_top_attackers(
                    start_time, filters
                )
            elif section == "Attack Patterns":
                data["sections"]["Attack Patterns"] = self._get_attack_patterns(
                    start_time, filters
                )
            elif section == "Event Logs":
                data["sections"]["Event Logs"] = self._get_event_logs(
                    start_time, filters
                )
            elif section == "Recommendations":
                data["sections"]["Recommendations"] = self._get_recommendations(
                    data["sections"]
                )
    ...
    def _get_executive_summary(self, start_time, filters):
        ...
        threat_levels = self.db.query(
            PacketLog.threat_level, func.count(PacketLog.id)
        ).filter(PacketLog.timestamp >= start_time)
        if filters.get("protocol") and filters["protocol"] != "ALL":
            threat_levels = threat_levels.filter(
                PacketLog.protocol == filters["protocol"]
            )

        threat_distribution = {
            level: count
            for level, count in threat_levels.group_by(PacketLog.threat_level).all()
            if level
        }
        ...
        return {
            ...
            "safety_score": (
                round(((total_events - malicious_events) / total_events * 100), 2)
                if total_events > 0
                else 100
            ),
    ...
    def _get_geographic_distribution(self, start_time, filters):
        ...
        countries = self.db.query(PacketLog.country, func.count(PacketLog.id)).filter(
            PacketLog.timestamp >= start_time, PacketLog.country.isnot(None)
        )
    ...
    def _get_top_attackers(self, start_time, filters):
        ...
        query = self._apply_filters(query, filters)
        results = (
            query.group_by(PacketLog.src_ip)
    ...
    def _get_attack_patterns(self, start_time, filters):
        query = self.db.query(PacketLog.attack_type, func.count(PacketLog.id)).filter(
            PacketLog.timestamp >= start_time, PacketLog.attack_type.isnot(None)
        )
        query = self._apply_filters(query, filters)
    ...
    def _get_recommendations(self, current_sections):
        ...
        if exec_summary and exec_summary.get("malicious_events", 0) > 100:
            recs.append(
                "High malicious activity detected. Increase firewall strictness."
            )
