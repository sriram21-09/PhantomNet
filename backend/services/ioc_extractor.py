...
class IOCExtractor:
    ...
    def classify_threat_type(self, ioc_value: str, ioc_type: str, context: str = "") -> str:
        ...
        if ioc_type == "ips": return "Suspicious IP"
        if ioc_type in ["md5", "sha1", "sha256"]: return "Malware Hash"
        if ioc_type == "domains": return "Suspicious Domain"
        return "Generic Threat"

    def calculate_confidence(self, honeypot_count: int, external_confirmed: bool = False) -> str:
        """
    ...
    def process_event(self, event_data: Dict[str, Any], external_confirmed: bool = False) -> List[Dict[str, Any]]:
        """
        ...
        for ioc_type, values in extracted.items():
            for value in values:
                ...
                confidence = self.calculate_confidence(honeypot_count, external_confirmed)
