...
class NodeManager:
    ...
    def register_node(self, hostname: str, ip_address: str, honeypot_type: str) -> HoneypotNode:
        """Registers a new honeypot node or updates an existing one."""
        node_id = str(uuid.uuid4())[:8]
        
        # Check if node with same hostname/IP exists
        existing_node = self.db.query(HoneypotNode).filter(
            (HoneypotNode.hostname == hostname) | (HoneypotNode.ip_address == ip_address)
        ).first()
    ...
    def get_node_by_id(self, node_id: str):
        ...
        return self.db.query(HoneypotNode).filter(HoneypotNode.node_id == node_id).first()
