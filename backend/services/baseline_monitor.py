    ...
    def _analyze_baseline(self):
        ...
        try:
            ...
            current_activity = (
                ...
                .filter(PacketLog.timestamp >= last_min_start)
                .scalar()
                or 0
            )
            ...
            if not self.baseline_calculated:
                ...
                total_in_window = (
                    db.query(func.count(PacketLog.id))
                    .filter(PacketLog.timestamp >= window_start)
                    .scalar()
                    or 0
                )
                ...
                logger.info(
                    f"Established baseline: {self.average_events_per_minute:.2f} events/min"
                )
            ...
            if current_activity > threshold:
                ...
                logger.warning(
                    f"Traffic spike detected: {current_activity} (Baseline: {self.average_events_per_minute:.2f})"
                )
            ...
            if current_activity < threshold * 2:
                self.average_events_per_minute = (
                    self.average_events_per_minute * 0.95
                ) + (current_activity * 0.05)
