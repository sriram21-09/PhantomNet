...
logging.basicConfig(
    filename="/tmp/ssh_debug.log", level=logging.DEBUG, format="%(asctime)s %(message)s"
)
...
class SSHServer(paramiko.ServerInterface):
    ...
    def check_auth_password(self, username, password):
        logging.info(
            f"🚨 PASSWORD ATTACK: IP={self.client_ip} User={username} Pass={password}"
        )
...
def start_server():
    ...
    logging.info(
        "🪤 SMART SSH Honeypot Active on Port 2222 (Logging to DB & /tmp/ssh_debug.log)"
    )
