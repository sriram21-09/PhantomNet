...
async def test_invalid_login():
    ...
    with pytest.raises((asyncssh.PermissionDenied, asyncssh.misc.ConnectionLost)):
        await asyncssh.connect(
            HOST, PORT, username=INVALID_USER, password=INVALID_PASS, known_hosts=None
        )
...
async def test_valid_login():
    ...
    conn = await asyncssh.connect(
        HOST, PORT, username=VALID_USER, password=VALID_PASS, known_hosts=None
    )
    conn.close()
...
def test_asyncssh_login_logs_exist():
    ...
    login_logs = [log for log in logs if log.get("event") == "login_attempt"]
