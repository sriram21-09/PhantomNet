...
class ThreatInput(BaseModel):
    ...
    threat_score: float = Field(
        0.0, description="Baseline threat score from firewall/rules", example=10.0
    )
    is_malicious: bool = Field(
        False, description="Known malicious flag (e.g. from blacklist)", example=False
    )
    attack_type: str = Field(
        "UNKNOWN", description="Preliminary attack classification", example="UNKNOWN"
    )
    honeypot_type: str = Field(
        "NONE", description="Honeypot type interaction", example="HTTP"
    )
