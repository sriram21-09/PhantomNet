...
def register_node(req: NodeRegisterRequest, db: Session = Depends(get_db), api_key: str = Depends(get_api_key)):
...
def node_heartbeat(req: NodeHeartbeatRequest, db: Session = Depends(get_db), api_key: str = Depends(get_api_key)):
    manager = NodeManager(db)
...
def create_policy(req: PolicyCreateRequest, db: Session = Depends(get_db), api_key: str = Depends(get_api_key)):
    engine = PolicyEngine(db)
...
def assign_policy(req: PolicyAssignRequest, db: Session = Depends(get_db), api_key: str = Depends(get_api_key)):
    engine = PolicyEngine(db)
