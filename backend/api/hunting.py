...
def get_related_events(ip: Optional[str] = None, honeypot: Optional[str] = None, window: Optional[int] = 1440, db: Session = Depends(get_db)):
    service = HuntingService(db)
    return service.get_related_events(ip, honeypot, window)

@router.get("/history")
def get_search_history(db: Session = Depends(get_db)):
    return db.query(SearchHistory).order_by(SearchHistory.executed_at.desc()).limit(20).all()
    ...
    return [
        {
            ...
            "conditions": [
                ...
                {"field": "timestamp", "operator": "greater_than", "value": "24h_ago"} # Note: simplified for example
            ]
        },
        {
            ...
            "conditions": [
                ...
                {"field": "attack_type", "operator": "contains", "value": "Brute Force"}
            ]
        }
    ]
