...
class CrossValidator:
    ...
    def evaluate_model(self, model, X, y, model_name="Model"):
        ...
        print(f"✅ {model_name} - Mean Accuracy: {stats['accuracy_mean']:.4f} (+/- {stats['accuracy_std']:.4f})")
        return stats
...
def run_stability_test():
    ...
    enhanced_model = RandomForestClassifier(n_estimators=100, max_depth=12, random_state=42)
    enhanced_stats = validator.evaluate_model(enhanced_model, X_scaled, y, "AttackClassifierV3_Enhanced")

    # 2. Evaluate Baseline Model (Assume simpler params for baseline comparison)
    baseline_model = RandomForestClassifier(n_estimators=50, max_depth=5, random_state=42)
    baseline_stats = validator.evaluate_model(baseline_model, X_scaled, y, "AttackClassifier_Baseline")
