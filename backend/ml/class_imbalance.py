...
def compute_class_distribution(
    y: np.ndarray, class_names: Optional[Dict[int, str]] = None
) -> Dict[str, Any]:
    """
    ...
    distribution = {"total_samples": total, "classes": {}}
    ...
    if len(counts) >= 2:
        ...
        distribution["imbalance_ratio"] = (
            float(majority / minority) if minority > 0 else float("inf")
        )
    else:
...
def compute_per_class_metrics(
    y_true: np.ndarray, y_pred: np.ndarray, class_names: Optional[Dict[int, str]] = None
...
def assess_imbalance_impact(
    ...
    majority_ratio = max([c["ratio"] for c in distribution["classes"].values()])
    ...
    if attack_recall < 0.7:
        assessment["issues"].append(f"Attack class recall is low ({attack_recall:.2%})")
    ...
    if len(assessment["issues"]) == 0:
        ...
        assessment["verdict"] = (
            "Class imbalance IS materially affecting model behavior."
        )
...
def generate_imbalance_report(
    ...
    for name, stats in distribution["classes"].items():
        lines.append(
            f"  {name.upper()}: {stats['count']} samples ({stats['percentage']:.1f}%)"
        )
