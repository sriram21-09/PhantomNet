...
def compute_misclassification_indices(
    ...
    return {"fp_indices": fp_indices, "fn_indices": fn_indices}
...
def analyze_error_distribution(
    ...
    result = {
        "summary": {
            ...
            "error_rate": (
                (len(fp_indices) + len(fn_indices)) / len(y_true_arr)
                if len(y_true_arr) > 0
                else 0
            ),
        },
    ...
    if len(fp_indices) > 0:
        ...
        if y_proba is not None:
            ...
            result["false_positives"]["confidence_stats"] = _compute_confidence_stats(
                fp_proba
            )
    else:
    ...
    if len(fn_indices) > 0:
        ...
        if y_proba is not None:
            ...
            result["false_negatives"]["confidence_stats"] = _compute_confidence_stats(
                fn_proba
            )
    else:
...
def get_error_samples(
    ...
    result["error_type"] = [
        "FP" if y_true_arr[i] == 0 else "FN" for i in sample_indices
    ]
...
def generate_error_summary(analysis: Dict[str, Any]) -> str:
    ...
    if fp.get("confidence_stats"):
        ...
        lines.append(
            f"  Mean: {cs['mean_confidence']:.3f}, Low-conf: {cs['low_confidence_count']}, High-conf: {cs['high_confidence_count']}"
        )
