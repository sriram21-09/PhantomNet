...
class FeatureExtractorV2:
    ...
    SENSITIVE_FILES = {
        "/etc/passwd", "/etc/shadow", "/etc/group", "id_rsa", ".ssh/",
        "config.php", "settings.py", ".env", "web.config", "boot.ini"
    }
    ...
    def extract_v2_features(self, event: dict) -> Dict[str, float]:
        ...
        # Update trackers
        self.ip_events[src_ip].append({"ts": timestamp, "data": raw_data, "dst": event.get("dst_ip")})
        ...
        features["avg_command_length"] = statistics.mean([len(c) for c in commands]) if commands else 0.0
        
        # 2. Shell Escapes
        features["shell_escape_count"] = float(sum(1 for char in raw_data if char in self.SHELL_ESCAPES))
        
        # 3. Directory Traversal
        features["directory_traversal_count"] = float(raw_data.count("../") + raw_data.count("..\\"))
        ...
        features["lateral_movement_index"] = len(self.ip_destinations[src_ip]) / len(self.ip_events[src_ip])
        
        # 10. Sensitive File Access
        features["sensitive_file_count"] = float(sum(1 for f in self.SENSITIVE_FILES if f in raw_data))
    ...
    def _calculate_entropy(self, data: str) -> float:
        if not data:
            return 0.0
        probs = [f/len(data) for f in defaultdict(int, {c: data.count(c) for c in set(data)}).values()]
    ...
    def _calculate_interval_variance(self, src_ip: str) -> float:
        ...
        if len(events) < 3:
            return 0.0
        intervals = [(events[i+1]["ts"] - events[i]["ts"]).total_seconds() for i in range(len(events)-1)]
