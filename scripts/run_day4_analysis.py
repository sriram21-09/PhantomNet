...
def run_error_analysis(X_test, y_test, y_pred, y_proba):
    ...
    # Get sample errors
    fp_samples = get_error_samples(
        X_test, y_test, y_pred, error_type="fp", max_samples=5
    )
    fn_samples = get_error_samples(
        X_test, y_test, y_pred, error_type="fn", max_samples=5
    )
