...
async def test_sharing_workflow():
    ...
    for ioc in iocs:
        print(
            f"  - {ioc['type']}: {ioc['value']} ({ioc['threat_type']}, confidence: {ioc['confidence']})"
        )
    ...
    if stix_bundle:
        print(
            "STIX Bundle generated successfully (JSON length: ", len(stix_bundle), ")"
        )
    ...
    report_path = threat_intel_service.generate_daily_report(
        iocs, "reports/test_daily_feed.json"
    )
