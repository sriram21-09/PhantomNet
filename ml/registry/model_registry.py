...
class ModelRegistry:
    ...
    def _parse_version(self, version: str) -> tuple:
        ...
        if not match:
            raise ValueError(f"Invalid version format: {version}. Expected v{{major}}.{{minor}}.{{patch}}")
    ...
    def get_model_by_status(self, status: str) -> Optional[Dict[str, Any]]:
        ...
        index = self._load_index()
        matching_models = [m for m in index["models"].values() if m.get("status") == status]
        if not matching_models:
            return None
            
        matching_models.sort(key=lambda x: self._parse_version(x["version"]), reverse=True)
