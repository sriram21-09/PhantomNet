...
class CredentialTrapSystem:
    ...
    def generate_honeytoken(self, token_type="generic", source="honeypot-1"):
        usernames = ["admin", "root", "ubuntu", "ec2-user", "dbadmin", "svc_account", "backup_user"]
        username = random.choice(usernames) + "_" + ''.join(random.choices(string.ascii_lowercase + string.digits, k=4))
    ...
    def monitor_usage(self, username, password):
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        cursor.execute('SELECT id, usage_count FROM honeytokens WHERE username = ? AND password = ?', (username, password))
    ...
    def seed_config_file(self, file_path, token_type="config"):
        username, password = self.generate_honeytoken(token_type=token_type, source=file_path)
...
if __name__ == "__main__":
    ...
    trap_system.seed_config_file("C:/Users/vivekananda reddy/PhantomNet/data/fake_db_config.ini")
