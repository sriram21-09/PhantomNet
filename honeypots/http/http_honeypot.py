...
class DeceptiveHTTPHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if self.path in ["/admin", "/wp-admin", "/phpmyadmin"]:
            ...
            self.end_headers()
            self.wfile.write(
                f"<html><body><h1>Admin Login</h1><form>User: <input><br>Pass: <input type='password'></form></body></html>".encode()
            )
            return
        ...
        if self.path == "/files/":
            ...
            self.end_headers()
            self.wfile.write(
                b"Index of /files/<br><a href='backup.sql'>backup.sql</a><br><a href='config.php.bak'>config.php.bak</a>"
            )
            return
